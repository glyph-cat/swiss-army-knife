## 0.0.0-experimental.91
- Fixed a bug in `VideoCamera` where it could not be properly disposed

## 0.0.0-experimental.90
- Removed: `AggregateWatcher`
- Rewritten: `Watcher` (it now has the same implementation has Cotton Box's `Watcher`)
- `removeDuplicates` now accept an optional second parameter for extracting unique values from arrays containing complex data types.

## 0.0.0-experimental.89
- New helper: `getPaginationSpecs` for calculating start and end indices for paginated lists
- New helper: `withFallbackRenderKey` that injects a manually provided fallback key to React Nodes if they do not already have one specified in the props
- New hooks: `useStableCallback`, `useDebouncedCallback`
- Renamed: `PaperSingleSideLayout` -> `PrintPaperSingleSideLayout`
- Renamed: `PaperDoubleSideLayout` -> `PrintPaperDoubleSideLayout`
- Renamed: `getPageLayoutOrder` -> `getPrintPageLayoutOrder`
- `sleepSort` has been split into `@glyph-cat/sleep-sort`
- Removed legacy "Size Aware" APIs
- New helper types: `Fn`, `Factory`
- Deprecated: `TypedFunction` (please use `Fn` instead)
- Rewritten `useConstructor`
- Added: `DivMod`, `divMod`
- Removed: `ReadonlyOrWritableArray`, `DivModStruct`, `createDivModData`, `useEffectValue`, `useLayoutEffectValue`, `useClickAwayListener_UNSAFE`, `usePointerLeaveListener_UNSAFE`
- Rewritten: `<TextEmphasis>` + `<MultiPatternTextEmphasisProps>` (for more complex use cases)
- Rewrite of Input Focus API:
  - It is now dissociated from `<CoreUIProvider>`.
  - Removed: `InputFocusTracker`, `IAutoFocusableProps`, `IFocusableElementWithEventListener`, `useCommonFocusableRefHandler` (no replacement, input focus is now auto-detected, globally)
  - Added in Core package:
    - `AnyInputFocusState` which is a read-only state manager that keeps track of whether any inputs are in focus (`true` | `false`)
    - `manuallyRegisterInputFocus`, `manuallyRegisterInputBlur`
  - Replaced `checkInputFocus()` (from React package) with `AnyInputFocusState.get()` (in Core package)
  - Replaced `useCheckInputFocus()` with `useAnyInputFocusState()`
- Removed from `@glyph-cat/swiss-army-knife`: `Nullable`, `ExtendedCSSProperties`
- The following items have been moved from `@glyph-cat/swiss-army-knife` to `@glyph-cat/foundation`: `Empty`, `NotImplementedError`, `UnsupportedPlatformError`, `InternalError`, `HTML_ELEMENT_TAGS`, `HTMLElementTag`, `createRef`
- Fixed a bug where the `h1` to `h6` elements are malformed in `HTML_ELEMENT_TAGS` and `HTMLElementTag`
- The following items have been split into `@glyph-cat/type-checking`: `isBoolean`, `isFunction`, `isNaN`, `isNull`, `isNullOrEmpty`, `isNullOrWhitespace`, `isNullOrUndefined`, `isNumber`, `isNumberOrNaN`, `isObject`, `isObjectOrNull`, `isString`, `isSymbol`, `isThenable`, `isUndefined`, `isUndefinedOrNull`
- The following items have been split into `@glyph-cat/css-utils`: `CSSProperties`, `CSSPropertiesExtended`, `CustomCSSVariablesRecord`, `querySelectorLast`, `em`, `fr`, `percent`, `pt`, `px`, `rem`, `vh`, `vmax`, `vmin`, `vw`, `tryFormatAsClassName`, `StyleMap`, `StyleManager`, `serializePixelValue`, `normalizeCSSValue`, `mapPropertyNameFromJSToCSS`, `injectInlineCSSVariables`, `injectCSSVariables`, `convertStyleObjectPropertyKeys`, `compileStyleObjectToString`, `compileStyle`, `compileCSSVariables`, `addStyles`
- Rewritten Color API as separate package `@glyph-cat/color`

## 0.0.0-experimental.88
- The video element of `VideoCamera` is no longer appended to `document.body`; unless the `appendVideoElement` parameter in the constructor is `true`.
- `getAngleOfVectorsIn2D` and `getAngleFromPointsIn2D` no longer changes the output result into positive when it is supposedly negative.
- Added `LinearEquation2D` class.
- These functions have been replaced:
  - `createLinearEquationFromPointAndGradient` -> `LinearEquation2D.fromPointAndGradient`
  - `createLinearEquationFromTwoPoints` -> `LinearEquation2D.fromPoints`
  - `linearMapFromPointAndGradient` -> `LinearEquation2D.calcFromPointAndGradient`
  - `linearMapFromTwoPoints` -> `LinearEquation2D.calcFromPoints`
- Added new error class: `InternalError`
- Added helper classes: `EventManager`, `RuntimeStorage`, `UrlSanitizer`
- Added helper functions: `getNthTriangularNumber`, `isClamped`, `getPageLayoutOrder`
- Added string helper `splice`
- Added conversion helpers: `celsiusToFahrenheit`, `fahrenheitToCelsius`, `cmToInches`, `inchesToCm`
- Added `SmartView` component and hook `useSmartViewContext`. For now, this is a variant of `View` that is self-aware of its depth of nesting within another SmartView.
- Updated list of material symbol names.
- Stability improvements for `tryOnly` in asynchronous use cases.
- `tryOnly` now returns a boolean indicating whether the invocation of the callback was successful.
- Added hook `useStateAlt`, an equivalent to `useStateWithReset` that was removed in an earlier version.
- Added hook `useMemoAlt` that is similar to [`useMemo`](https://react.dev/reference/react/useMemo) but accepts a third optional parameter that allows custom dependency list comparison
- Added hooks: `useHydrationState`, `useMountedState`, `useCheckStrictMode`.
- For web environments, `useMediaQuery` and `useColorScheme` now internally uses `useHydrationState`.
- `ClientOnly` now renders its children based on `useHydrationState`; in React Native, it will behave like a `<Fragment>`, allowing children to render immediately.
- New `<DeferRendering>` component that defers children from rendering by one cycle regardless of environment.
- `ExtendedCSSProperties` has been renamed to `CSSPropertiesExtended` but the old export is still available
- "Size Aware" API has been rewritten, old APIs are still available with the `Legacy` suffix.
- Marked as deprecated: `useComponentId`.
- Moved from `@glyph-cat/swiss-army-knife` to `@glyph-cat/foundation` (new package): `BuildType`, `Encoding`, `ShortBool`, `KeyValuePair`, `StrictPropertyKey`, `PlainRecord`, `StrictRecord`, `PartialStringRecord`, `NumericRecord`, `StringRecord`, `PartialRecord`, `ReadonlyOrWritableArray`, `TypedFunction`, `CleanupFunction`, `LenientString`, `NumericValues3`, `NumericValues4`, `Awaitable`, `Voidable`, `PossiblyUndefined`, `Awaited`, `AwaitedReturnType`, `Falsable`, `IDisposable`, `RefObject`, `QRErrorCorrectionLevel`, `HttpMethod`, `HttpStatus`, `Dimension2D`, `Value2D`, `Value3D`, `Tuple2D`, `Tuple3D`, `Charset`.
- For simplicity and safety, `createDebouncedCallback` now does not return any value.
- `createDebouncedPromise` has been removed.
- `useMergedRefs` can now accept any number of refs instead of just 2.
- `memoize` now accepts an optional second parameter for custom comparison logic of the callback arguments.
- `useClickAwayListener` now no longer needs a dependency array (old implementation available as `useClickAwayListener_UNSAFE`)
- `usePointerLeaveListener` has been renamed to `usePointerLeaveListener_UNSAFE` and is replaced by `useMoveAwayListener` (which also no longer needs a dependency array).
- Deprecated: `PaymentPointerProtector`, `PaymentPointer`, `usePaymentPointer`, `PaymentPointerProps`
- Removed: `AdjustableDelay`, `areDepsEqual`
- Removed: `encodeUTF16`, `decodeUTF16`. Please use [`TextDecoder`](https://developer.mozilla.org/en-US/docs/Web/API/TextDecoder) instead
- Added back `sleepSort`.

## 0.0.0-experimental.87
- The behavior of `isObject` has been modified to return `true` only when the value is an object and not `null`.
- Added `isObjectOrNull` as a replacement for the old `isObject`.
- `isObjectNotNull` has been deprecated and replaced by `isObject`.
- Added "Size Aware" API.
- The default color of `ProgressRing` has changed from primary color to `'#808080'`.
- `isJSONequal` has migrated to `'@glyph-cat/equality'`.
- `ExperimentalSizeObserver` is now marked as deprecated.

## 0.0.0-experimental.86
* `PreloadableComponent` to fall back to `renderToString` from `'react-dom/server'` instead of the client-side-optimized version so that errors can be suppressed gracefully.

## 0.0.0-experimental.85
* Fixed an issue where `'react-dom/client'` is mistakenly bundled along with the package.

## 0.0.0-experimental.84
...
* New client-side-optimized alternative of `renderToString` from `'react-dom/server'`.
