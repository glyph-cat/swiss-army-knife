...
- The following items have been moved from `@glyph-cat/swiss-army-knife` to `@glyph-cat/foundation`: `Empty`, `NotImplementedError`, `UnsupportedPlatformError`, `InternalError`, `HTML_ELEMENT_TAGS`, `HTMLElementTag`
...
- The following items have been split into `@glyph-cat/css-utils`: `CSSProperties`, `CSSPropertiesExtended`, `CustomCSSVariablesRecord`, `querySelectorLast`, `em`, `fr`, `percent`, `pt`, `px`, `rem`, `vh`, `vmax`, `vmin`, `vw`, `tryFormatAsClassName`
