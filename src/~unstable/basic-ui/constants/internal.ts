...
import { BasicUIFlow, BasicUILayout, BasicUIPosition, BasicUISize } from '../abstractions'
...
// #endregion Meter-based components
...
export const meterBasedSizePresets: Record<BasicUISize, number> = {
  's': 12,
  'm': 20,
  'l': 32,
} as const

export function createMeterBasedLayoutPresets(
  layoutH: string,
  layoutV: string,
): Readonly<Record<BasicUILayout, [className: string, maskAngle: number]>> {
  return {
    'horizontal': [layoutH, 90],
    'vertical': [layoutV, 0],
  }
}
