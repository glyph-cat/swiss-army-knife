## v0.10.0
- Removed cotton-box as dependency from react-test-utils

## v0.9.0
* The `values` property has been renamed to `get`, while it is still available, it has been marked as deprecated and will be removed in future versions.
* Added optional property `wrapper` that allows test component of the hook to be wrapped in another component, such as a Provider.
* `HookTester` now has a new `forceUpdate` method.
